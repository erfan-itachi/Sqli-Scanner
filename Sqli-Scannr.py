    ...
    try:
        for phase in (GET, POST):
            ...
            for match in re.finditer(r"((\A|[?&])(?P<parameter>[^_]\w*)=)(?P<value>[^&#]+)", current):
                ...
                print(Fore.MAGENTA + "[*] scanning %s parameter '%s'" % (phase, match.group("parameter")))
                ...
                for (dbms, regex) in ((dbms, regex) for dbms in DBMS_ERRORS for regex in DBMS_ERRORS[dbms]):
                    if not vulnerable and re.search(regex, content[HTML], re.I) and not re.search(regex, original[HTML], re.I):
                        print(Fore.GREEN + "\n[*] %s parameter '%s' appears to be error SQLi vulnerable (%s)" % (phase, match.group("parameter"), dbms))
                ...
                for prefix, boolean, suffix, inline_comment in itertools.product(PREFIXES, BOOLEAN_TESTS, SUFFIXES, (False, True)):
                    if not vulnerable:
                        ...
                        if all(_[HTTPCODE] and _[HTTPCODE] < http.client.INTERNAL_SERVER_ERROR for _ in (original, contents[True], contents[False])):
                            if any(original[_] == contents[True][_] != contents[False][_] for _ in (HTTPCODE, TITLE)) or bytes(DBMS_ERRORS.encode() in retval[HTML]):
                                vulnerable = True
                            else:
                        ...
                        if vulnerable:
                            print(Fore.GREEN + "\n[*] %s parameter '%s' appears to be blind SQLi vulnerable (e.g.: '%s')"  % (phase, match.group("parameter"), payloads[True]))
                            retval = True
        if not usable:
...
if __name__ == "__main__":
    ...
    if options.url:
        ...
        print(Fore.MAGENTA + "\n[*] scan results: %s vulnerabilities found" % (Fore.GREEN+"possible" if result else Fore.RED+"no"))
    else:
        parser.print_help()
